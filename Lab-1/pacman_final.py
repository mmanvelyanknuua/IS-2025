...
class GameRenderer:
    ...
    def tick(self, in_fps: int):
        ...
        while not self._done:
            ...
            if self._hero is None: self.display_text("YOU DIED", (self._width / 2 - 256, self._height / 2 - 256), 50)
            if self.get_won(): self.display_text("YOU WON", (self._width / 2 - 256, self._height / 2 - 256), 50)
            pygame.display.flip()
    ...
    def display_text(self, text, in_position=(32, 0), in_size=15):
...
class Ghost(MovableObject):
    ...
    def handle_mode_switch(self):
        if difficulty == 0:
            ...
            print(player_position, current_maze_coord)
            dist = euclidean_dist(player_position, current_maze_coord)
            if dist <= 5:
                hero_visible = True
            if hero_visible:
    ...
    def calculate_direction_to_next_target(self) -> Direction:
        if self.next_target is None:
            if self.mode == GhostBehaviour.CHASE and not self._renderer.is_kokoro_active():
    ...
    def request_path_to_player(self, in_ghost):
        if difficulty < 2:
            ...
            target_x = max(0, min(player_position[0] + dx, len(self.game_controller.numpy_maze[0]) - 1))
            target_y = max(0, min(player_position[1] + dy, len(self.game_controller.numpy_maze) - 1))
...
class Cookie(GameObject):
    def __init__(self, in_surface, x, y):
        super().__init__(in_surface, x, y, 2, (255, 255, 0), True)


class Powerup(GameObject):
    def __init__(self, in_surface, x, y):
        super().__init__(in_surface, x, y, 4, (255, 255, 255), True)
...
if __name__ == "__main__":
    difficulty = int(input("Choose difficulty: easy(0), medium(1) or hard(2): "))
    unified_size = 16
